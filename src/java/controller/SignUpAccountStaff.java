 ...
 * @author Admin
 */
public class SignUpAccountStaff extends HttpServlet {
            ...
            if (e == null) {
                if (p == null) {
                    if (pass.equals(re_pass)) {
                        dao.singupStaff(user, re_pass, email, phonenumber);
                        dao.addIDKhachHang(user);
                        response.sendRedirect("AddNewStaff.jsp");
                    } else {
                        request.setAttribute("mess", "pass is not the same as repass! ");
                        request.getRequestDispatcher("signupAccountstaff.jsp").forward(request, response);
                    }
